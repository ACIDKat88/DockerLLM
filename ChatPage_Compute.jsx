...
function ChatPage() {
          ...
          if (line) { // Only process non-empty lines
            ...
            try {
              ...
              if (parsed.token) {
                ...
                // **HANDLE SOURCES RECEIVED DURING STREAMING**
                console.log("[DEBUG] 📎 Received sources during streaming:", parsed.sources);
                
                // Process sources similar to fetchSources
                const sourcesData = parsed.sources;
                let sources = [];
                
                if (sourcesData && Array.isArray(sourcesData.pdf_elements)) {
                  console.log("[DEBUG] 📎 Processing pdf_elements:", sourcesData.pdf_elements.length, "elements");
                  sourcesData.pdf_elements.forEach((element, index) => {
                    console.log(`[DEBUG] 📄 PDF Element ${index + 1}:`, {
                      name: element.name,
                      title: element.title,
                      hasContent: !!element.content,
                      contentLength: element.content?.length || 0,
                      hasPdfUrl: !!element.pdf_url
                    });
                  });
                  
                  sources = sourcesData.pdf_elements.map((element, index) => {
                    // ENHANCED TITLE EXTRACTION for streaming sources
                    console.log(`[DEBUG] 📎 PROCESSING ELEMENT ${index + 1}:`, {
                      name: element.name,
                      title: element.title,  
                      pdf_url: element.pdf_url,
                      hasContent: !!element.content
                    ...
                    if (!title && element.pdf_url) {
                      ...
                      if (fileName && fileName !== '' && fileName !== 'undefined') {
                        ...
                        title = decodeURIComponent(fileName.replace(/\.pdf$/i, ''));
                        titleSource = "pdf_url";
                        console.log("[DEBUG] 📎 Extracted title from streaming pdf_url:", title);
                      }
                    }
                    
                    // If still no title, try to extract from content
                    if (!title && element.content) {
                      // Look for "Document: FILENAME" pattern in content
                      const docMatch = element.content.match(/Document:\s*([^\s]+\.pdf)/i);
                      if (docMatch && docMatch[1]) {
                        title = docMatch[1];
                        titleSource = "content";
                        console.log("[DEBUG] 📎 Extracted title from streaming content:", title);
                      }
                    }
                    ...
                    if (!title) {
                      ...
                      console.log("[DEBUG] 📎 Using fallback title for streaming:", title);
                    }
                    // DON'T convert "Unknown Document" to generic - keep backend-provided titles as-is
                    
                    console.log(`[DEBUG] 📎 FINAL TITLE for element ${index + 1}: "${title}" (source: ${titleSource})`);
                  ...
                  console.log("[DEBUG] 📎 Processed streaming sources with enhanced titles:", sources.map(s => ({ 
                    title: s.title, 
                    contentLength: s.content.length,
                    hasTitle: !s.title.startsWith('Document ')
                  })));
                }
                
                if (sources.length > 0) {
                  console.log("[DEBUG] 📎 Processing", sources.length, "sources from stream");
                  sourcesReceivedDuringStreaming = true; // Mark that sources were received
                  console.log("[DEBUG] 📎 🎯 SOURCES RECEIVED DURING STREAMING - Setting flag to prevent fallback");
                  
                  const pdf_elements = sources.map(source => ({
                    name: source.title, // Use the actual title from the source
                    display: "side",
                    pdf_url: source.pdf_url || '',
                    content: source.content || 'No content available'
                  }));
                  ...
                  // Update the current bot message with sources
                  setMessages(prev => {
                    const updated = [...prev];
                    if (updated[messageIndex]) {
                      updated[messageIndex].hasSources = true;
                      updated[messageIndex].sources = pdf_elements;
                      updated[messageIndex].sourcesMarkdown = sourcesMarkdownString;
                    }
                    return updated;
                  });
                  
                  // Update side panel data structures
                  const questionNumber = messages.filter(msg => msg.sender === 'user').length;
                  console.log("[DEBUG] 📊 Calculated question number for streaming sources:", questionNumber);
                  console.log("[DEBUG] 📊 Current messages count:", messages.length);
                  console.log("[DEBUG] 📊 User messages count:", messages.filter(msg => msg.sender === 'user').length);
                  console.log("[DEBUG] 📊 🎯 STREAMING SOURCES TITLES:", sources.map(s => `"${s.title}"`));
                  
                  setSourcesByQuestion(prev => {
                    console.log("[DEBUG] 📊 Updating sourcesByQuestion, previous state:", prev);
                    const updated = { ...prev };
                    if (!updated[questionNumber]) {
                      updated[questionNumber] = [];
                      console.log("[DEBUG] 📊 Created new question entry for question", questionNumber);
                    }
                    
                    const currentTitles = new Set(updated[questionNumber].map(s => s.title));
                    console.log("[DEBUG] 📊 🔍 Existing titles for question", questionNumber, ":", Array.from(currentTitles));
                    console.log("[DEBUG] 📊 🔍 About to add", sources.length, "STREAMING sources with titles:", sources.map(s => `"${s.title}"`));
                    
                    sources.forEach((source, idx) => {
                      if (!currentTitles.has(source.title)) {
                        updated[questionNumber].push(source);
                        console.log("[DEBUG] 📊 ✅ Added STREAMING source", idx + 1, "to question", questionNumber, ":", `"${source.title}"`);
                      } else {
                        console.log("[DEBUG] 📊 ⚠️ STREAMING source", idx + 1, "already exists for question", questionNumber, ":", `"${source.title}"`);
                      }
                    });
                    
                    console.log("[DEBUG] 📊 🎯 FINAL sourcesByQuestion for question", questionNumber, ":", updated[questionNumber]?.map(s => `"${s.title}"`));
                    console.log("[DEBUG] 📊 🎯 Total sources in question", questionNumber, ":", updated[questionNumber]?.length || 0);
                    return updated;
                  });
                  ...
                  // Update sources count immediately during streaming (finalization delayed)
                  setTimeout(() => {
                    const total = calculateTotalSources();
                    console.log("[DEBUG] 📊 Sources count calculated during streaming:", total);
                    console.log("[DEBUG] 📊 sourcesFinalized during calculation:", sourcesFinalized);
                    console.log("[DEBUG] 📊 Current sourcesByQuestion:", Object.keys(sourcesByQuestion).length, "questions");
                    console.log("[DEBUG] 📊 Current allUniqueSources:", allUniqueSources.length);
                    setSourcesCount(total);
                    // Don't interfere with aggressive refresh trigger (forceUpdate > 100)
                    setForceUpdate(prev => prev < 100 ? prev + 1 : prev);
                  }, 200); // Slightly longer delay to ensure state updates have propagated
                  
                  // Update lastInteractionData for feedback buttons
                  const newInteractionData = {
                    question: currentUserQuery,
                    answer: botMessage,
                    sources: pdf_elements,
                    sourcesMarkdown: sourcesMarkdownString,
                    model: selectedModel,
                    temperature: temperature,
                    dataset: dataset,
                    personality: persona,
  ...
  useEffect(() => {
    ...
    if (total > 0) {
      ...
      setSourcesCount(total);
    } else if (!isGenerating && (Object.keys(sourcesByQuestion).length === 0 || messages.length <= 1)) {
      // Only reset to 0 if we're not generating AND we truly have no data
      console.log("[DEBUG useEffect sourcesCount] ✅ Setting sources count to 0 (confirmed no sources - not generating, no data)");
      setSourcesCount(0);
    } else {
      console.log("[DEBUG useEffect sourcesCount] ⚠️ Skipping reset to 0 - likely incomplete data loading or mid-stream");
      console.log("[DEBUG useEffect sourcesCount] ⚠️ isGenerating:", isGenerating, "sourcesByQuestion keys:", Object.keys(sourcesByQuestion), "messages:", messages.length);
    }
  ...
  }, [messages, sourceContent, sourcesByQuestion, allUniqueSources, forceUpdate, isGenerating, sourcesFinalized, sourcesCount]);
