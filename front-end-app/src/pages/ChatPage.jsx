...
function ChatPage() {
  ...
  async function fetchSources(query, dataset) {
    try {
      const response = await fetch("http://localhost/api/sources", {
        method: "POST",
        headers: {
  ...
  const fetchChatHistories = async () => {
    try {
      const response = await fetch(
        'http://localhost/api/chat/histories',
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleSelectModel = (model) => {
    ...
    if (sessionToken) {
      ...
      const endpoint = "http://localhost/api/user/preferences";
  ...
  useEffect(() => {
    ...
    if (sessionToken) {
      ...
      axios.get("http://localhost/api/me", { headers })
        .then(response => {
        ...
        .then(response => {
          ...
          if (response.data && response.data.username) {
            ...
            return axios.get("http://localhost/api/current_user", { headers });
          }
        })
        .then(response => {
          if (!response) return;
  ...
  return (
    <div className="chat-container">
      {/* Component JSX here - this is already in the file */}
    </div>
  );
}
