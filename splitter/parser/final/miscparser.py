...
class MiscParser:
    def __init__(self, pdf_folder):
        ...
        self.generic_heading_regex = self.subsection_regex_b
        ...
        self.meta_pattern = re.compile(r"\bREF:|UNCLASSIFIED| SI 230-03|230|ENCLOSURE\s+[A-Z]\b", re.IGNORECASE)
    ...
    def parse_pdf_structure(self, pdf_text: str, document_title: str, document_filename: str) -> dict:
        ...
        for line in lines:
            ...
            if stripped_line and stripped_line == stripped_line.upper() and re.search('[A-Z]', stripped_line):
                ...
                current_sub = None
                continue
            # STYLE B – Subsection (bullet point)
            ...
            if sub_b_match:
                ...
                if current_sec is not None:
                    ...
                    if current_chap is not None:
                        ...
                        current_chap.setdefault("sections", []).append(current_sec)
                continue
            # STYLE B – Section (uppercase words followed by a colon)
            sec_b_match = self.section_regex_b.match(stripped_line)
            if sec_b_match:
                sec_title = sec_b_match.group("title").strip()
                sec_content = sec_b_match.group("content").strip()
                ...
                current_sub = None
                continue
            # STYLE B – Chapter (title ending with acronym in parentheses)
