...
def process_node(node, parent_metadata, collection, pbar):
    ...
    Each node's metadata inherits the parent's metadata (including the document hash)
    and adds its own specific hash based on its type.
    """
    # Inherit parent's metadata, ensuring the top-level document hash is preserved.
    node_metadata = {
        **parent_metadata,
        "node_type": node.get("node_type", ""),
        "number": node.get("number", ""),
        "title": node.get("title", ""),
        # Explicitly include the document hash from the parent.
        "hash_document": parent_metadata.get("hash_document", node.get("hash_document", ""))
    }
    
    node_type = node.get("node_type", "").lower()
    # Determine the expected hash key based on the node type.
    if node_type == "document":
        specific_hash_key = "hash_document"
    elif node_type == "chapter":
        specific_hash_key = "hash_chapter"
    elif node_type == "section":
        specific_hash_key = "hash_section"
    elif node_type == "subsection":
        specific_hash_key = "hash_subsection"
    else:
        specific_hash_key = "hash"
    
    # Retrieve the node's own hash value: try the specific key, then fall back to a generic "hash".
    node_hash = node.get(specific_hash_key) or node.get("hash", "")
    
    # Store the node's own hash in its metadata under both the specific key and a generic key.
    node_metadata[specific_hash_key] = node_hash
    node_metadata["hash"] = node_hash
    ...
    if node_content:
        ...
        # Use the node's own hash as the document ID if available, otherwise generate a new UUID.
        doc_id = node_hash if node_hash else str(uuid.uuid4())
        collection.add(
