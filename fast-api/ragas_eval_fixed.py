...
try:
    ...
    async def _patched_generate(self, messages, stop=None, **kwargs):
        ...
        try:
            ...
            # Handle single tuple message
            elif isinstance(messages, tuple) and len(messages) == 2 and messages[0] == 'content':
                print("[PATCH] Converting tuple message to HumanMessage")
            ...
            # Handle list of messages with problematic types
                ...
                new_msgs = []
                for m in messages:
                    if isinstance(m, tuple) and len(m) == 2 and m[0] == 'content':
                        # Handle tuple format
            ...
            # Last resort: pass through to original
            return await _original_generate(self, messages, stop=stop, **kwargs)
...
def evaluation_worker(question, answer, contexts, temp_file_path):
    ...
    try:
        ...
        # Initialize LLM for metrics evaluation
        try:
            from langchain_community.chat_models import ChatOllama
            llm = ChatOllama(
                model="qwen3-ragas",
            ...
            print("[Subprocess] Successfully initialized LLM for metric evaluation")
        except Exception as e:
            print(f"[Subprocess] Error initializing LLM: {e}")
            # Create a dummy LLM
            from langchain.llms.fake import FakeListLLM
            llm = FakeListLLM(responses=["The answer is relevant. Score: 0.75"])
            print("[Subprocess] Using dummy LLM as fallback")
        ...
        # Use LLM directly without wrapper since it's causing issues
        evaluator_llm = llm
        print("[Subprocess] Using LLM directly without wrapper")
        
        # Helper function to safely score with a metric
        def safe_metric_score(metric_obj, sample_dict):
            try:
                print(f"[Subprocess] Trying to score with {type(metric_obj).__name__}")
                print(f"[Subprocess] Sample keys: {list(sample_dict.keys())}")
                
                # Wrap the LLM so it never returns a raw StringPromptValue
                if hasattr(metric_obj, 'llm'):
                    try:
                        import importlib
                        wrapped = None
                        for mod in ("ragas.llms", "ragas.llms.llm"):
                            try:
                                m = importlib.import_module(mod)
                                cls = getattr(m, "LangchainLLM", None)
                                if cls:
                                    wrapped = cls(metric_obj.llm)
                                    print(f"[Subprocess] Wrapped metric LLM via {mod}.LangchainLLM")
                                    break
                            except ImportError:
                                continue
                        if wrapped:
                        ...
                        print(f"[Subprocess] Could not wrap metric LLM: {wrap_err}")
                
                # Use the utility function to prepare the sample
                modified_sample = prepare_sample_for_ragas(sample_dict)
                
                # Call the appropriate score method
                if hasattr(metric_obj, 'score'):
                    score = metric_obj.score(modified_sample)
                    print(f"[Subprocess] Score result: {score}")
                    return score
                else:
                    print(f"[Subprocess] No score method found on metric")
                    return None
            except Exception as e:
                print(f"[Subprocess] Error in safe_metric_score: {e}")
                import traceback
                traceback.print_exc()
                return None
        
        # Compute metrics one by one
        print("[Subprocess] Computing individual metrics...")
        
        # Faithfulness
        try:
            faithfulness_metric = Faithfulness(llm=evaluator_llm)
            results["faithfulness"] = safe_metric_score(faithfulness_metric, sample)
            if results["faithfulness"] is not None:
                print(f"[Subprocess] Computed faithfulness: {results['faithfulness']}")
            else:
                ...
                results["faithfulness"] = safe_metric_score(faithfulness_metric, minimal_sample)
                if results["faithfulness"] is not None:
                    print(f"[Subprocess] Second attempt faithfulness: {results['faithfulness']}")
                else:
                    print("[Subprocess] Both attempts for faithfulness failed")
        except Exception as e:
            print(f"[Subprocess] Error setting up faithfulness: {e}")
        
        # Context Precision 
        try:
            context_precision = LLMContextPrecisionWithoutReference(llm=evaluator_llm)
            results["context_precision"] = safe_metric_score(context_precision, sample)
            if results["context_precision"] is not None:
                print(f"[Subprocess] Computed context_precision: {results['context_precision']}")
            else:
                # Try with minimal sample
                minimal_sample = {
                    "user_input": question,
                ...
                results["context_precision"] = safe_metric_score(context_precision, minimal_sample)
                if results["context_precision"] is not None:
                    print(f"[Subprocess] Second attempt context_precision: {results['context_precision']}")
                else:
                    print("[Subprocess] Both attempts for context_precision failed")
        except Exception as e:
            print(f"[Subprocess] Error setting up context_precision: {e}")
        
        # Context Recall
        try:
            context_recall = LLMContextRecall(llm=evaluator_llm)
            results["context_recall"] = safe_metric_score(context_recall, sample)
            if results["context_recall"] is not None:
                print(f"[Subprocess] Computed context_recall: {results['context_recall']}")
            else:
                # Try with minimal sample
                minimal_sample = {
                    "user_input": question,
                ...
                results["context_recall"] = safe_metric_score(context_recall, minimal_sample)
                if results["context_recall"] is not None:
                    print(f"[Subprocess] Second attempt context_recall: {results['context_recall']}")
                else:
                    print("[Subprocess] Both attempts for context_recall failed")
        except Exception as e:
            print(f"[Subprocess] Error setting up context_recall: {e}")
        
        # Answer Relevancy / Response Relevancy
        try:
            answer_relevancy = ResponseRelevancy(llm=evaluator_llm, embeddings=embeddings)
            results["answer_relevancy"] = safe_metric_score(answer_relevancy, sample)
            if results["answer_relevancy"] is not None:
                print(f"[Subprocess] Computed answer_relevancy: {results['answer_relevancy']}")
            else:
                # Try with minimal sample
                minimal_sample = {
                    "user_input": question,
                ...
                results["answer_relevancy"] = safe_metric_score(answer_relevancy, minimal_sample)
                if results["answer_relevancy"] is not None:
                    print(f"[Subprocess] Second attempt answer_relevancy: {results['answer_relevancy']}")
                else:
                    print("[Subprocess] Both attempts for answer_relevancy failed")
        except Exception as e:
            print(f"[Subprocess] Error setting up answer_relevancy: {e}")
        
        # Context Entity Recall (optional additional metric)
        try:
            entity_recall = ContextEntityRecall(llm=evaluator_llm)
            results["entity_recall"] = safe_metric_score(entity_recall, sample)
            if results["entity_recall"] is not None:
                print(f"[Subprocess] Computed entity_recall: {results['entity_recall']}")
            else:
                # Try with minimal sample
                minimal_sample = {
                    "user_input": question, 
                ...
                results["entity_recall"] = safe_metric_score(entity_recall, minimal_sample)
                if results["entity_recall"] is not None:
                    print(f"[Subprocess] Second attempt entity_recall: {results['entity_recall']}")
                else:
                    print("[Subprocess] Both attempts for entity_recall failed")
        except Exception as e:
            print(f"[Subprocess] Error setting up entity_recall: {e}")
...
async def compute_metrics_with_evaluator(question: str, answer: str, contexts: List[str], reference: Optional[str] = None) -> Dict[str, float]:
    ...
    try:
        ...
        try:
            ...
            llm = get_model()
            print("[DEBUG] Using model:", type(llm).__name__)
            
            # Wrap the LLM with LangchainLLM if available
            evaluator_llm = llm
            if 'LangchainLLM' in globals() and LangchainLLM is not None:
                try:
                    evaluator_llm = LangchainLLM(llm)
                    print(f"[DEBUG] Wrapped evaluator LLM with LangchainLLM")
                except Exception as wrap_err:
                    print(f"[DEBUG] Could not wrap evaluator LLM: {wrap_err}")
            ...
            print(f"[DEBUG] Error initializing LLM: {llm_err}")
        ...
        # Compute metrics one by one
        print("[DEBUG] Computing individual metrics...")
        
        # Faithfulness 
        try:
            faithfulness_metric = Faithfulness(llm=evaluator_llm)
            results["faithfulness"] = await safe_metric_score(faithfulness_metric, sample)
            if results["faithfulness"] is not None:
                print(f"[DEBUG] Computed faithfulness: {results['faithfulness']}")
            else:
                print(f"[DEBUG] Both attempts for faithfulness failed")
        except Exception as e:
            print(f"[DEBUG] Error setting up faithfulness: {e}")
        
        # Context Precision
        try:
            context_precision = LLMContextPrecisionWithoutReference(llm=evaluator_llm)
            results["context_precision"] = await safe_metric_score(context_precision, sample)
            if results["context_precision"] is not None:
                print(f"[DEBUG] Computed context_precision: {results['context_precision']}")
            else:
                print(f"[DEBUG] Both attempts for context_precision failed")
        except Exception as e:
            print(f"[DEBUG] Error setting up context_precision: {e}")
        
        # Context Recall
        try:
            context_recall = LLMContextRecall(llm=evaluator_llm)
            results["context_recall"] = await safe_metric_score(context_recall, sample)
            if results["context_recall"] is not None:
                print(f"[DEBUG] Computed context_recall: {results['context_recall']}")
            else:
                print(f"[DEBUG] Both attempts for context_recall failed")
        except Exception as e:
            print(f"[DEBUG] Error setting up context_recall: {e}")
        ...
        try:
            ...
            results["answer_relevancy"] = await safe_metric_score(answer_relevancy, sample)
            if results["answer_relevancy"] is not None:
                print(f"[DEBUG] Computed answer_relevancy: {results['answer_relevancy']}")
            else:
                print(f"[DEBUG] Both attempts for answer_relevancy failed")
        except Exception as e:
            print(f"[DEBUG] Error setting up answer_relevancy: {e}")
...
async def compute_ragas_metrics(question: str, answer: str, contexts: List[str], reference: Optional[str] = None) -> Dict[str, float]:
    ...
    # Finally try the custom implementation using Ollama
    try:
        from langchain_community.chat_models import ChatOllama
        model = get_model()
