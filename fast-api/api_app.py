...
try:
    ...
    if result.returncode != 0:
        print("× Ollama is not installed or not in PATH. RAGAS metrics will use LLMEvaluator implementation.")
    else:
        ...
        if result.returncode != 0:
            print("× Ollama server is not running. RAGAS metrics will use LLMEvaluator implementation.")
        else:
...
async def chat_stream(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    async def token_generator():
        ...
        try:
            if RAGAS_AVAILABLE and dataset_option != "None" and retrieved_docs:
                # Check if we already have RAGAS metrics from our synchronous evaluation
