...
try:
    ...
    if result.returncode != 0 or "Ollama is running" not in result.stdout:
        print("× Ollama server is not running or not reachable. RAGAS metrics will use LLMEvaluator implementation.")
    else:
...
app.mount("/src", StaticFiles(directory="/app/frontend"), name="src")
...
#     """
#     ollama_base_url = "https://docker-llm-ollama--6b5efca2ab.jobs.scottdc.org.apolo.scottdata.ai/"

#     print(f"[DEBUG] Loading model: {model_name} with temperature: {temperature}")
#     return ChatOllama(
